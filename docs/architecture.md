...
flowchart LR
    subgraph "Phase 1: EXTRACT"
        RAW[raw/ source] --> HASH{Hash check}
        HASH -->|unchanged| SKIP[Skip]
        HASH -->|new/changed| EXTRACT[LLM extracts concepts, entities, facts]
    end
...
**Phase 1** uses the Researcher persona to extract structured data. **Phase 2** uses the Compiler persona to generate pages. Hash-based skip ensures sources aren't reprocessed unnecessarily.
...
flowchart TB
    ...
    RAW --> SKILL[/wiki-ingest skill]
    SKILL --> WIKI[wiki/ compiled pages]
...
All capture channels feed into a single FastAPI endpoint. The API writes standardized markdown files to `raw/`. The `/wiki-ingest` skill processes them into wiki pages.
