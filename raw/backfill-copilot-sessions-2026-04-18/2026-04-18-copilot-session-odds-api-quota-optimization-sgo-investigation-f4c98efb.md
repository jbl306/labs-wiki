...
**Checkpoint title:** Odds API quota optimization, SGO investigation
**Session ID:** `31d93251-dac7-4543-89a4-bd567847c867`
**Checkpoint file:** `/home/jbl/.copilot/session-state/31d93251-dac7-4543-89a4-bd567847c867/checkpoints/010-odds-api-quota-optimization-sg.md`
**Checkpoint timestamp:** 2026-04-13T00:37:23.827121Z
...
<history>
1. User reported props in fallback state, hit rate discrepancy, requested deep dive on dashboard metrics
   - Diagnosed props showing April 7 data due to missing ensemble model artifacts (7/9 stats)
   - Diagnosed MLflow DNS failure during April 12 weekly retrain (container was restarting)
   - Diagnosed Odds API returning 401 Unauthorized on all requests
   - Confirmed hit rate discrepancy (51.46% vs 52.52%) is different populations, NOT computation bug — 100% agreement on overlapping data
   - Implemented predictor fallback loading: CatBoost > XGBoost > LightGBM > Ridge when ensemble pkl missing
   - Implemented MLflow resilience: `_ensure_mlflow()` tests connectivity, falls back to local file tracking
   - Fixed `/prop-hit-rate` endpoint mixing data sources (snapshot counts with matview hit rates)
   - Wrote 9 tests, all passing
   - Generated 5,160 predictions for today, dashboard now shows April 12
   - Created PR #38, merged, rebuilt and deployed containers
   - Added 4 lessons to tasks/lessons.md

2. User asked about SGO data completeness and API call waste — 500 calls/month Odds API budget
   - Launched explore agent to audit all external API call sites
   - Found dashboard/BFF make ZERO external API calls (DB-only reads) — not the problem
   - Found pipeline (07:00 UTC) AND props-refresh (22:00 UTC) BOTH fetch props — double-dipping
   - Found `ODDS_API_MARKETS_PER_REQUEST=3` causes 3 requests per game instead of 1
   - Calculated: ~51 calls/day × 30 = ~1,530 calls/month on 500/month budget

3. User asked to check Odds API key (eb4a4ca82b5709fde429d542cb9bfcc0)
   - Tested key directly — HTTP 200, key is valid
   - But `x-requests-used: 500, x-requests-remaining: 0` — quota fully exhausted
   - Quota resets 1st of each month at 12am UTC

4. User confirmed to implement quota optimizations
   - Removed prop fetch from morning pipeline (main.py line 845-852)
   - Changed `ODDS_API_MARKETS_PER_REQUEST` from 3 → 8 (config.py)
   - Changed `ODDS_API_MIN_REQUESTS_REMAINING` from 5 → 50 (config.py)
   - Added quota usage logging to prop_lines.py
   - Created PR #39, merged, rebuilt and deployed container
   - Verified new config active in container

5. User asked about SGO API — should still be getting data there
   - Tested SGO API directly — HTTP 200, returns data
   - Found "Response is missing 11797 bookmaker odds. Upgrade your API key" notice
   - SGO returns ~618 odds per game with player props for all stats
   - Was mid-investigation of SGO bookmaker data structure when compaction triggered
   - SGO_DK dead since April 10, SGO_FD only 1 line on April 12 despite API having data
   - Need to debug why the SGO fetcher isn't extracting lines properly
</history>
...
1. April 11 training interrupted mid-ensemble assembly — only pts and fg3m ensemble pkls saved, 7 stats had base models only
2. April 12 weekly retrain failed because MLflow container was restarting (DNS resolution failure for nba-ml-mlflow)
3. Odds API key valid but 500/500 quota exhausted — 401 on all prop/game line requests since ~April 4-5
...
- Quota resets: 1st of each month at 12:00 AM UTC

**Hit rate — three different views, all correct:**
...
- 100% agreement when computed on same overlapping data (6,328 rows)

**SGO API status (NEEDS INVESTIGATION):**
...
- The `bookOdds` field value might be a string not a dict (saw `AttributeError: 'str' object has no attribute 'keys'` during investigation)
...
- `NBA_ML_ENGINE_PATH` is relative (`../../nba-ml-engine`) — use absolute path override when needed
...
- No `game_schedule` table exists — use `game_lines` for game data
