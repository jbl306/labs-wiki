...
**Checkpoint title:** Pipeline resilience fixes, dashboard metrics investigation
**Session ID:** `31d93251-dac7-4543-89a4-bd567847c867`
**Checkpoint file:** `/home/jbl/.copilot/session-state/31d93251-dac7-4543-89a4-bd567847c867/checkpoints/009-pipeline-resilience-fixes-dash.md`
**Checkpoint timestamp:** 2026-04-12T23:02:46.533439Z
...
<history>
1. Previous sprints (48-53) were completed and merged — Sprint 53 as PR #37
   - Added edge_abs display, SHAP post-training hook, scale-aware edge monitoring, Grafana matview dashboard, CI lint
   - All deployed to homelab

2. User reported: "Why are props in a fallback state? Why is hit rate different on props history P&L vs backtesting page? Do a deep dive on accuracy of metrics across the dashboard and fix"
   
   - **Investigated props fallback**: Found `getFeaturedPropSlate()` (index.ts:308-332) falls back to last date where `prop_lines JOIN predictions` has data. That date was April 7 (5 days stale)
   
   - **Discovered root cause chain**:
     a. **Missing ensemble models**: Only `pts/ensemblemodel.pkl` and `fg3m/ensemblemodel.pkl` exist on disk. All other stats (reb, ast, blk, tov, fg_pct, ft_pct, stl) have base models (catboost, xgboost, etc.) but NOT the ensemble pickle. The model_registry DB has entries for all, but files are missing (likely OOM/interrupted during previous training Apr 11)
     b. **Weekly retrain failed today** (Apr 12 16:00 UTC): MLflow container (`nba-ml-mlflow`) DNS resolution failed from `nba-ml-api` container during training. Training aborted at Step 1 (minutes model) after 5 minutes of retries. Error: `NameResolutionError: Failed to resolve 'nba-ml-mlflow'`
     c. **Odds API returning 401 Unauthorized**: All prop line fetching from the-odds-api.com is failing. Only SGO (SportsGameOdds) props are coming through, and only 1 PRA line for today
   
   - **Investigated hit rate discrepancy**:
     - History P&L (`/api/props/history`): 3,360 bets, 51.46% hit rate — uses `prop_line_snapshots` with `settled_at IS NOT NULL`, joined with `prediction_blend` CTE
     - Backtest (`/api/backtest`): 9,483 bets, 52.52% hit rate — uses `mv_backtest_summary` materialized view from `prop_lines + predictions + game_logs`
     - **Critical finding**: When computed on the SAME data (6,328 rows with both sources), settlement methods agree 100% (zero disagreements, 3,372 hits each = 53.3%). The ~1% difference is entirely due to different bet populations, NOT different computation logic
   
   - **Implemented fix 1 — Predictor model fallback**: Added `_try_fallback_model()` to `src/inference/predictor.py`. When production ensemble model file is missing, tries CatBoost > XGBoost > LightGBM > Ridge from the same stat directory. Successfully loaded fallback models for all stats
   
   - **Implemented fix 2 — MLflow resilience**: Added `_ensure_mlflow()` to `src/training/trainer.py`. Tests MLflow connectivity; on failure, switches to local file-based tracking (`file:///tmp/mlflow-fallback`) so training continues without experiment logging. Replaced all 3 `mlflow.set_tracking_uri(config.MLFLOW_TRACKING_URI)` calls with `_ensure_mlflow()`
   
   - **Triggered predictions**: Ran `python main.py predict --store` with fallback-enabled predictor. Generated 5,160 predictions (up from 1,032) — all 10 stats now have 516 predictions each for today
   
   - **Verified dashboard**: `featured_date` changed from `2026-04-07` to `2026-04-12` (today), `uses_fallback_date: false`. But only 1 prop shown (the single PRA from SGO_FD) because Odds API is 401
   
   - **Wrote tests**: 9 tests in `tests/test_pipeline_resilience.py` — all pass
   
   - **Was beginning full metrics audit** across all dashboard endpoints when compaction triggered

3. Created fix branch: `fix/pipeline-resilience-dashboard-metrics`
</history>
...
<technical_details>
**Pipeline failure chain (3 cascading issues):**
1. Weekly retrain (Apr 12 16:00 UTC) failed at Step 1 because MLflow DNS couldn't resolve `nba-ml-mlflow` — container was restarting at that exact time. Training aborted with exit code 1 after 5m9s of retries
2. Previous retrain (Apr 11) partially completed: saved base models (catboost, xgboost, etc.) for all stats but ensemble assembly (`ensemblemodel.pkl`) only succeeded for pts and fg3m. Likely OOM-killed during ensemble assembly (known: needs 14GB+)
3. Odds API key returning 401 Unauthorized on all requests — no new prop lines from DK/FD sources since April 4-5. Only SGO sources still working (SGO_FD has 1 PRA line for today)

**Model registry vs disk state:**
...
- stl's production model is CatBoostModel (not ensemble), so it was always working

**Hit rate computation — two systems that agree:**
...
- History has fewer bets because it requires `prop_line_snapshots` with settlement; Backtest requires `game_logs` actual stats
...
- MLflow is reachable NOW (tested: HTTP 200), was transiently unavailable during training

**Data state (April 12, 2026):**
...
- Backtest matview: 9,483 total bets, 52.5% overall hit rate
...
- BFF has 2-minute TTL cache on history endpoint, 5-minute cache on backtest
