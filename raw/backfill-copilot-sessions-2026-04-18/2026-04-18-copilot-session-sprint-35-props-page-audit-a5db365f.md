...
**Checkpoint title:** Sprint 35 props page audit
**Session ID:** `8d466128-8017-482d-b021-0fffe970d5eb`
**Checkpoint file:** `/home/jbl/.copilot/session-state/8d466128-8017-482d-b021-0fffe970d5eb/checkpoints/003-sprint-35-props-page-audit.md`
**Checkpoint timestamp:** 2026-03-29T02:33:05.463725Z
...
<history>
1. Sprint 34 was already in progress (chart accuracy fixes) when context was compacted
   - Completed all 8 fixes: PRA hit_rate=0, model-health 50s hang, hardcoded season, missing fg_pct/ft_pct in hit rate, backtest ROI=0, avg_edge showing MAE, edge distribution buckets, edge summary filter investigation
   ...
   - All 11 Sprint 34 todos marked done

2. User requested Sprint 35: Props page enhancements
   - Features requested: (a) Historical table of "today's most confident predictions" on Props page, configurable to drill down by stat/date, (b) Running P&L and bet sizing using Kelly Criterion, (c) Prop hit column on the props table
   - Invoked `execute-sprint-from-report` skill
   - Created branch `feature/sprint-35-props-historical-pnl`
   - Launched two parallel explore agents to audit: (1) Props page code, Charts, App routing, UI libraries (2) BFF endpoints, DB schema, settlement data
   - Both agents completed successfully with comprehensive results
   - **Context compaction occurred before implementation began**
</history>
...
- API on port 8000, BFF on port 8501, dashboard serves on 8501
...
- **Dynamic season**: `getCurrentNbaSeason()` in BFF — month >= 10 means current year, else previous year. Format: `YYYY-YY`

### Sprint 35 Audit Findings (for implementation)
...
- No Kelly/bankroll/bet-size logic exists anywhere in codebase

**BFF `/api/props` endpoint** (lines 508-588):
...
- Each prop has: player_name, team, source, stat_name, line, predicted_value, confidence_score/low/high/tier, edge_pct, edge_value, direction, call, game_date, odds, opponent, is_home, game_time

**BFF `/api/backtest` endpoint** (lines 826-915):
...
- BacktestDetail type exists but `details` array is returned empty `[]`

**BFF `/api/dashboard` best_predictions** (lines 312-361):
- 8-table join, confidence-ranked, edge-policy-filtered
- Deduped to 10 by player+stat (highest confidence kept)
- Confidence: logistic approximation using CI width vs line distance

**BFF caching**: In-memory Map with TTL (dashboard=2min, backtest=5min, edge-policy=10min)

**PropsPage current state** (237 lines):
...
- No historical data, no P&L, no hit column, no Kelly sizing

**Routes** (App.tsx): /, /props, /rankings, /injuries, /seasons, /models, /health, /waiver, /backtest, /reference, /player/:id
