...
**Checkpoint title:** Sprint 50 audit complete, ready to implement
**Session ID:** `31d93251-dac7-4543-89a4-bd567847c867`
**Checkpoint file:** `/home/jbl/.copilot/session-state/31d93251-dac7-4543-89a4-bd567847c867/checkpoints/002-sprint-50-audit-complete-ready.md`
**Checkpoint timestamp:** 2026-04-12T11:37:00.266140Z
...
<history>
1. Sprint 48 was implemented and merged as PR #30 (prior session)
   - Per-stat calibration, vig-aware thresholds, promotion gate
   - All changes deployed and validated

2. Sprint 49 was implemented and merged as PR #31 (prior session)
   - Sprint skill improvements (5 gaps closed in SKILL.md)
   ...
   - 30-day backtest: 53.2% overall hit rate

3. User asked to implement Sprint 49 next steps (current session — Sprint 50)
   - Invoked execute-sprint-from-report skill
   - Read Sprint 49 results report, AGENTS.md, lessons.md
   - Confirmed server mode (hostname beelink-gti13)
   - Ran data quality gate: game_lines has no `game_time` column (schema different from playbook SQL), zero 0.0 prop lines, only 1 PRA prop for today
   - Queried PTS recent performance: hit rates 42-58% over last 2 weeks, with suspicious avg_edge_pct of 0.97-1.21 on Apr 1 and Mar 30 (likely alt-line contamination)
   - Launched two explore agents in parallel:
     - **audit-pts-underperformance**: Completed with comprehensive findings
     - **audit-data-bugs**: Completed with comprehensive findings
   - **Current state: Audits complete, ready to plan and implement Sprint 50**
</history>
...
- Backtest command: `docker exec nba-ml-api python main.py backtest --start YYYY-MM-DD --end YYYY-MM-DD --min-edge 0.005`
...
- SKILL.md data quality gate SQL for timezone check is wrong for this schema (references non-existent `game_time` column)

**PTS Edge Calculation Problem:**
...
- PTS is NOT in LOW_R2_STATS (feature_selector.py:23) — uses all ~350 features

**SGO Timezone Bug Details:**
...
- Fix: add `.astimezone(ZoneInfo("America/New_York"))` before `.date()`

**BFF Alt-Line Bug Details:**
...
- Only hit rate trend endpoint still affected
