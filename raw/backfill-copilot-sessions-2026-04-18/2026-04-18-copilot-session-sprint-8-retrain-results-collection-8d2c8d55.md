...
**Checkpoint title:** Sprint 8 retrain results collection
**Session ID:** `8ddcc5a0-c1f3-4397-8f42-320e5da91f6e`
**Checkpoint file:** `/home/jbl/.copilot/session-state/8ddcc5a0-c1f3-4397-8f42-320e5da91f6e/checkpoints/005-sprint-8-retrain-results-colle.md`
**Checkpoint timestamp:** 2026-03-21T09:44:44.516543Z
...
6. User asked to run full Priority 1 retrain and evaluate all next priorities for Sprint 9
   ...
   - Restarted retrain with fix — ran for ~6+ hours

7. Monitoring retrain progress (main activity this segment)
   - Monitored training via `read_bash` over many polling cycles
   - Training completed 8/9 stats: pts, reb, ast, stl, blk, tov, fg_pct, ft_pct
   - fg3m was NOT trained (original retrain process was killed or stuck before reaching it)
   - Discovered multiple stale processes in container: original full retrain (22+ hours CPU), daily Ofelia pipeline, stuck MLflow query, duplicate fg3m retrains
   - Cleaned up by finding container-internal PIDs via `/proc` filesystem and killing stale processes
   - Started clean fg3m retrain (`python main.py train --stat fg3m`) — currently running
   - Collected all results from model_registry database table
   - Sprint 8 report update in progress (have all data, need to write it)

8. Sprint 9 priority evaluation (completed via explore agent earlier)
   - Recency weighting: NOT implemented — HIGH impact
   - Pace × usage interaction: Partial — LOW effort
   - CatBoost: NOT in requirements — ensemble diversity
   - Team name mapping: Duplicated in game_lines.py and prop_lines.py — needs centralization
   - Ensemble meta-learner: Ridge with 3-fold OOF — no recency decay
</history>
...
- **Feature selection per stat**: stl=233/363, blk=196/363, fg_pct=233/363, ft_pct=241/363
...
- Critically: previous Ensemble dominated because leaky features + default params created useful diversity; with tuned params, LightGBM/XGBoost now win most stats individually
...
- The PIDs shown by `docker top` are HOST PIDs, not container PIDs — can't use them with `os.kill()` inside container

### Daily Pipeline Conflict
...
- **For Sprint 9**: Need to handle Optuna timeout for daily pipeline (5-6 hour retrain too long for daily runs)
...
- MLflow registered_models table is EMPTY — models only in model_registry table

### model_registry Table Schema
```sql
model_registry: id, model_name, version, trained_at, metrics (JSON), artifact_path, is_production, config_snapshot
```
...
- For Ensemble models, also includes `meta_weight_{base_model}` entries
