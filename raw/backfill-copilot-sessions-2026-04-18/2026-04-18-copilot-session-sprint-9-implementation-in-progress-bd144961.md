...
**Checkpoint title:** Sprint 9 implementation in progress
**Session ID:** `8ddcc5a0-c1f3-4397-8f42-320e5da91f6e`
**Checkpoint file:** `/home/jbl/.copilot/session-state/8ddcc5a0-c1f3-4397-8f42-320e5da91f6e/checkpoints/006-sprint-9-implementation-in-pro.md`
**Checkpoint timestamp:** 2026-03-21T11:40:57.841951Z
...
<history>
1. User asked to create a PR for feature/sprint-7-browser-backfill and merge to main
...
2. User asked to create progress tracker, implement Sprint 9, commit, test, fix, validate, then evaluate and document
   - Created branch `feature/sprint-9-model-improvements` from main
   - Launched 3 parallel explore agents to research: recency weighting, CatBoost/LSTM, Optuna/team mapping/pace features
   - All 3 agents completed with detailed findings
   - Created SQL todos (8 items with 7 dependencies)
   - Updated plan.md with Sprint 9 plan

3. Started implementing Sprint 9 changes (IN PROGRESS when compaction occurred):
   - Completed: team mapping centralization (created team_mapping.py, updated game_lines.py and prop_lines.py)
   - Completed: config changes (added PIPELINE_OPTUNA_TRIALS/TIMEOUT, USE_RECENCY_WEIGHTS, RECENCY_DECAY_LAMBDA)
   - Completed: CatBoost model created (catboost_model.py)
   - In progress: Updating all model train() signatures to accept sample_weight parameter
   - Completed: base.py abstract method updated
   - Completed: xgboost_model.py train() updated with sample_weight
   - Completed: lightgbm_model.py train() updated with sample_weight
   - NOT YET DONE: random_forest_model.py, ridge_model.py, ensemble.py, lstm_model.py train() signatures
   - NOT YET DONE: trainer.py (recency weight computation + passing weights to models)
   - NOT YET DONE: tuner.py (CatBoost Optuna space + add to registries)
   - NOT YET DONE: builder.py (pace × usage interaction features)
   - NOT YET DONE: main.py pipeline command (use lighter Optuna settings)
   - NOT YET DONE: requirements.txt (add catboost)
   - NOT YET DONE: ensemble.py (add CatBoost to base models, remove LSTM reference in trainer)
   - NOT YET DONE: Tests, deploy, retrain, report
</history>
...
<technical_details>
### Sprint 8 Retrain Results (completed before Sprint 9 work)
All 9 stats retrained with Sprint 8 fixes. Production models:
- pts: LightGBM (val_mse=44.18), reb: LightGBM (7.32), ast: Ensemble (4.18)
- stl: XGBoost (0.909), blk: LightGBM (0.722), tov: LightGBM (1.711)
- fg_pct: XGBoost (0.0354), ft_pct: XGBoost (0.0650), fg3m: LightGBM (1.744)

### Model Architecture for sample_weight
All sklearn/xgb/lgb models natively support `sample_weight` in their `.fit()` method:
...
- Ensemble: Needs to pass weights to base model `.train()` calls in K-fold loop

### Recency Decay Design
...
- Compute weights in trainer.py before calling model.train(), pass as parameter

### CatBoost Integration Points
1. `src/models/catboost_model.py` — Created, extends BaseModel
2. `src/training/tuner.py` — Need to add `_get_catboost_space()` and register in `_SPACE_FUNCS` and `_cls_map`
3. `src/training/trainer.py` — Need to add CatBoostModel to MODEL_CLASSES (line 67-73), remove LSTM append
4. `src/models/ensemble.py` — Need to add CatBoostModel to `_BASE_MODEL_CLASSES` (line 28-33)
5. `requirements.txt` — Need to add `catboost>=1.2.0`
...
- Alternatively, pass n_trials/timeout params through train_all → tune_model chain

### Key Patterns in Model Files
...
- LSTM conditionally imported via `try: import torch` in trainer.py (lines 76-80)

### Prop Lines Team Mapping Note
In prop_lines.py, the dict was a LOCAL variable inside a function (not module-level like game_lines.py). The import pattern differs: `from src.data.team_mapping import ODDS_NAME_TO_ABBR` (no underscore prefix), then `_full_to_abbr: dict[str, str] = dict(ODDS_NAME_TO_ABBR)`.

### Git State
- Branch: `feature/sprint-9-model-improvements` (created from main at `05f4d8e`)
- No commits yet on this branch
- Main is up to date with merged PR #6
