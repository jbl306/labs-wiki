...
**Checkpoint title:** VS Code agents, instructions, and Android capture setup
**Session ID:** `208e4513-1b23-4977-9336-ddeaa74d2d4a`
**Checkpoint file:** `/home/jbl/.copilot/session-state/208e4513-1b23-4977-9336-ddeaa74d2d4a/checkpoints/002-vs-code-agents-instructions-an.md`
**Checkpoint timestamp:** 2026-04-07T15:48:37.006662Z
...
<history>
1. User asked to create custom agents, skills, and instructions for all wiki management aspects using VS Code best practices and context size management
   - Explored full project structure via subagent (found existing: 6 skills in .github/skills/, 4 agent personas in agents/, 4 templates, 6 docs, copilot-instructions.md at 208 lines)
   - Fetched VS Code Copilot customization docs from Context7 (learned .agent.md, .instructions.md, .prompt.md formats)
   ...
   - Slimmed `.github/copilot-instructions.md` from 208 → 56 lines
   - Updated `setup.sh` to validate new directories
   - Verified with `setup.sh` — all green

2. User reported unknown tool error for `runTerminalCommand` and asked to update model from GPT-5.2 to GPT-5.4
   - Researched valid VS Code agent tools via Context7
   - Discovered `editFiles` and `runTerminalCommand` are internal runtime tool names (appear in hooks), NOT valid for `tools:` frontmatter
   - Valid tools: `search`, `search/codebase`, `search/usages`, `web`, `web/fetch`, `agent`, `vscode/askQuestions`
   - File editing and terminal are default agent capabilities (always available)
   - Fixed all 7 agents: removed invalid tools, updated model to GPT-5.4
   - Saved lesson to OpenMemory

3. User asked for high-level usage documentation
   - Rewrote `docs/tool-setup.md` comprehensively: context architecture table, all 7 agents with invoke examples, scoped instructions explanation, prompt files, common workflows ("I found an article", "status check", "fill gaps", "end-of-day"), valid tool identifiers reference, expanded troubleshooting

4. User asked to update capture-sources doc with detailed Android HTTP Shortcuts setup
   - Added comprehensive step-by-step Android section to `docs/capture-sources.md`
   - Initially used `{{sharing_text}}`/`{{sharing_title}}` as "built-in" variables — this was WRONG

5. User reported error: "Variable error: tried to get value of variable that doesn't exist: sharing_text"
   - Researched HTTP Shortcuts official docs at https://http-shortcuts.rmy.ch/variables
   - Learned: there are NO built-in sharing variables. Must create GLOBAL VARIABLES with "Allow Receiving Value from Share Dialog" enabled
   - Global vars use single braces `{var_name}` (purple), local vars use double braces `{{var_name}}` (orange)
   - Completely rewrote Android section with correct approach

6. User reported 400 Bad Request: "type must be: url, text, or note"
   - Diagnosed: shared content from Chrome contains special chars (quotes, newlines, query params) that break JSON when raw-substituted via `{shared_text}` placeholder
   - Solution: build JSON body in scripting block using `JSON.stringify()` for proper escaping, store in `request_body` variable, use `{request_body}` as the entire body
   - Updated docs with scripting-based approach, merged URL-vs-text detection into main flow

7. User reported CURRENT ERROR: "unexpected json error. Expected ':' at line 1 column 15 path $.request_body"
   - This means HTTP Shortcuts is sending the literal text `{request_body}` as JSON — the variable placeholder isn't being resolved
   - The body field contains `{request_body}` but HTTP Shortcuts is treating it as literal JSON text rather than resolving the variable
   - THIS IS THE ACTIVE BUG being debugged
</history>
...
- File editing and terminal commands are DEFAULT agent capabilities — always available without listing
...
- Handoff pattern: `handoffs: [{label, agent, prompt, send}]`

### VS Code Scoped Instructions
- File: `.github/instructions/*.instructions.md`
- Frontmatter: `applyTo: "glob-pattern"` — auto-loads when matching files are open
- Searched in locations defined by `chat.instructionsFilesLocations` setting

### HTTP Shortcuts Android App (CRITICAL LESSONS)
...
- Better approach: build JSON in scripting block with `JSON.stringify()`, store in variable, pass through

### CURRENT BUG: `{request_body}` not resolving
...
- Another possible cause: when request body type is "Custom Text" with content type application/json, HTTP Shortcuts might try to validate/parse it as JSON before variable substitution
...
- Auth: Bearer token via `WIKI_API_TOKEN` env var
