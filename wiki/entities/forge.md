---
title: "Forge"
type: entity
created: 2026-04-08
last_verified: 2026-04-08
source_hash: "a5ed6720a8e499fec504f09a5c3ce60d6067385dc5b5038864e2b0f8372284f3"
sources:
  - raw/2026-04-08-the-strategic-human-gate-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-toolkit-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-token-economy-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-specialized-review-principle-10-claude-code-principles.md
  - raw/2026-04-08-10-claude-code-principles-what-the-research-actually-says-10.md
quality_score: 100
concepts:
  - forge
related:
  - "[[The Hardening Principle]]"
  - "[[The Token Economy Principle]]"
  - "[[10 Claude Code Principles | What the Research Actually Says]]"
tier: hot
tags: [llm, tool, automation]
---

# Forge

## Overview

Forge is an open-source Claude Code plugin designed to assemble science-backed specialist agent teams for production-grade workflows. It encodes the ten Claude Code Principles into actionable automation.

## Key Facts

| Field | Value |
|-------|-------|
| Type | Tool |
| Created | Unknown |
| Creator | JD Forsythe |
| URL | https://github.com/jdforsythe/forge |
| Status | Active |

## Relevance

Implements the research-backed principles for agentic workflow design, enabling reproducible, efficient, and observable LLM-powered systems.

## Associated Concepts

- **[[The Hardening Principle]]** — Implements hardening of agent workflows.
- **[[The Token Economy Principle]]** — Optimizes agent team size and token usage.

## Related Entities

No related entities documented yet.

## Sources

- [[10 Claude Code Principles | What the Research Actually Says]] — where this entity was mentioned
- [[The Specialized Review Principle | 10 Claude Code Principles]] — additional source
- [[The Token Economy Principle | 10 Claude Code Principles]] — additional source
- [[The Toolkit Principle | 10 Claude Code Principles]] — additional source
- [[The Strategic Human Gate Principle | 10 Claude Code Principles]] — additional source
