---
title: "jig"
type: entity
created: 2026-04-08
last_verified: 2026-04-08
source_hash: "a5ed6720a8e499fec504f09a5c3ce60d6067385dc5b5038864e2b0f8372284f3"
sources:
  - raw/2026-04-08-the-toolkit-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-token-economy-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-context-hygiene-principle-10-claude-code-principles.md
  - raw/2026-04-08-10-claude-code-principles-what-the-research-actually-says-10.md
quality_score: 85
concepts:
  - jig
related:
  - "[[The Context Hygiene Principle]]"
  - "[[10 Claude Code Principles | What the Research Actually Says]]"
tier: hot
tags: [llm, tool, context]
---

# jig

## Overview

jig is an open-source session profile manager that loads only the tools each project needs, supporting context hygiene and workflow specialization. It is designed to enforce the Claude Code Principles in daily development.

## Key Facts

| Field | Value |
|-------|-------|
| Type | Tool |
| Created | Unknown |
| Creator | JD Forsythe |
| URL | https://github.com/jdforsythe/jig |
| Status | Active |

## Relevance

Automates context management and tool selection, reducing context bloat and improving workflow reliability.

## Associated Concepts

- **[[The Context Hygiene Principle]]** — Helps manage and minimize context window usage.

## Related Entities

No related entities documented yet.

## Sources

- [[10 Claude Code Principles | What the Research Actually Says]] — where this entity was mentioned
- [[The Context Hygiene Principle]] — additional source
- [[The Token Economy Principle | 10 Claude Code Principles]] — additional source
- [[The Toolkit Principle | 10 Claude Code Principles]] — additional source
