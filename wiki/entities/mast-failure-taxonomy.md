---
title: "MAST Failure Taxonomy"
type: entity
created: 2026-04-08
last_verified: 2026-04-08
source_hash: "e07e5051691c4755e2f30694f3fc063b4f6c821a142667606bf4f3c54ed9a4a1"
sources:
  - raw/2026-04-08-the-strategic-human-gate-principle-10-claude-code-principles.md
  - raw/2026-04-08-the-observability-imperative-10-claude-code-principles.md
  - raw/2026-04-08-the-specialized-review-principle-10-claude-code-principles.md
quality_score: 100
concepts:
  - mast-failure-taxonomy
related:
  - "[[The Specialized Review Principle]]"
  - "[[The Specialized Review Principle | 10 Claude Code Principles]]"
tier: hot
tags: [multi-agent-systems, failure-modes, code-review]
---

# MAST Failure Taxonomy

## Overview

The MAST Failure Taxonomy is a classification system for common failure modes in multi-agent workflows, such as 'rubber-stamp reviews' where agents approve code without substantive analysis.

## Key Facts

| Field | Value |
|-------|-------|
| Type | Framework |
| Created | Unknown |
| Creator | Unknown |
| URL | N/A |
| Status | Active |

## Relevance

Helps teams identify and avoid structural pitfalls in agent-based review systems, supporting the implementation of the Specialized Review Principle.

## Associated Concepts

- **[[The Specialized Review Principle]]** — Addresses and mitigates failure modes documented in the taxonomy.

## Related Entities

No related entities documented yet.

## Sources

- [[The Specialized Review Principle | 10 Claude Code Principles]] — where this entity was mentioned
- [[The Observability Imperative | 10 Claude Code Principles]] — additional source
- [[The Strategic Human Gate Principle | 10 Claude Code Principles]] — additional source
