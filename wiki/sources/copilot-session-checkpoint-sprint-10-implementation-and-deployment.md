...
related:
  ...
  - "[[Homelab]]"
tier: archive
tags: [copilot-session, checkpoint, fileback, durable-knowledge, homelab, nba-ml-engine, dashboard, nba, machine learning, feature engineering, model tuning, deployment]
...
- Infrastructure: .gitignore, Dockerfile git, quantile test fix
...
- **catboost not in local venv initially**: Had to `pip install catboost` to fix tuner test imports. Already in requirements.txt and Docker image Environment:
