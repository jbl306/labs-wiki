...
- User invoked `/execute-sprint-from-report` to implement Sprint 23's immediate and high-priority next steps as Sprint 24:
- Created branch `feature/sprint-24-prop-consolidation-and-hit-rates`
- **SGO_FD stat misclassification investigation and fix** (DONE):
...
- Fix: added `threepointers`, `threepointersmade`, `madethrees`, `fg3m`, `3pm`, etc. to the map ### BFF → FastAPI Consolidation
...
- Pydantic `PropEdge` model now has 20 fields (was 10) to match what the frontend needs ### NaN Handling
...
- Pydantic rejects `nan` for `int | None` fields — need explicit NaN → None conversion via `_safe_int()` ### Model Selection
...
- Degradation guardrail also uses the configured metric for comparison ### Prop Hit-Rate
...
- A "hit" = the model's predicted direction (OVER/UNDER based on edge sign) matched the actual outcome ### Homelab Deployment
...
| Field | Value |
|-------|-------|
| Original | `raw/backfill-copilot-sessions-2026-04-18/2026-04-18-copilot-session-sprint-24-prop-consolidation-implementation-e5c3d4c3.md` |
| Type | checkpoint |
| Author | Unknown |
| Date | 2026-03-26 |
| URL | N/A |
