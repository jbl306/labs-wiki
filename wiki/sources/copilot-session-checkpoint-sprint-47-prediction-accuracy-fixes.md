...
- Fix negative prediction clamping (clamp before CI)
- Fix game_lines timezone off-by-one
- Fix prop line default 0.0 value
- Fix BFF alt-line DISTINCT ON queries (2 locations)
- Fix BFF hit rate query deduplication
- Fix BFF settlement DISTINCT ON
...
- blk: 55.4% (83 props) — above vig, small sample **Inverse Edge-Accuracy (ROOT CAUSE of lagging vig):**
- 0-1 edge: 53.4% (3088 bets) — BEST bucket
- 1-2 edge: 52.5% (640 bets)
- 2-3 edge: 48.5% (198 bets) — below random!
- 5+ edge: 47.9% (163 bets) — WORST This means the old 8% threshold was filtering OUT the best bets and keeping the worst. **Schema details:**
...
- Container workdir is `/app`, Python 3.12.13 **Environment:**
...
- API key auth blocks evaluation endpoints (NBA_ML_API_KEY env var empty) **Key gotchas discovered:**
...
- fg_pct/ft_pct have no prop lines but still train models — already excluded via EXCLUDED_PROP_STATS **Audit report files saved to /tmp (may be expired):**
...
| Field | Value |
|-------|-------|
| Original | `raw/backfill-copilot-sessions-2026-04-18/2026-04-18-copilot-session-sprint-47-prediction-accuracy-fixes-6c547374.md` |
| Type | checkpoint |
| Author | Unknown |
| Date | 2026-04-06 |
| URL | N/A |
